...
- (void)nextAction {
    NSDictionary *dic = @{@"deviceType":@"iPhone6,2",
                          ...
                          @"request":@{@"codeType":@1,
                                       @"phone":@"",
                                       @"email":[NSString stringWithFormat:@"%@", _phoneNumber],
                                       @"veriCode":[NSString stringWithFormat:@"%@", _verifyCodeTextField.text]}
                          };
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:nil];
    NSString *str = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    [SX_DataRequest POSTRequestWithString:@"http://appapi2.gamersky.com/v2/CheckCode" body:str block:^(id result) {
        ...
        if ([[result objectForKey:@"errorCode"] isEqual:@0]) {
            ...
            UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                registerVC.varifyToken = [result objectForKey:@"VeriFyToken"];
